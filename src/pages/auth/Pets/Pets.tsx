...
export default function Pets() {
    ...
    const imageBodyTemplate = (product: { avatar: string | undefined; name: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | null | undefined; }) => {
        return (

            <div className="flex align-items-center gap-2">
                <img src={product.avatar} alt={product.avatar} className="w-10 rounded" />
    ...
    const handlePetTemplate = (product) => {
        return (
            <div className="flex align-items-center gap-2">
                <button onClick={() => showMenu(true)} className="p-button font-bold bg-blue-600 p-2" >
                    <FaEye color="white" />
                </button>
                <div className="p-button font-bold bg-orange-600 p-2">
                    <FaPenSquare color="white" />
                </div>
                <button className="p-button font-bold bg-red-600 p-2" onClick={deleteItem(product.id)}>
                    <FaTimes color="white" />
                </button>
            </div>
        );
    ...
    useEffect(() => {
        PetsService.getPetsOfCompany().then(data => setProducts(data));

        // Ici, vous pouvez appeler votre API pour récupérer les détails de l'animal en utilisant l'ID de l'animal dans match.params.animalId
        // Exemple :
                                ...
                                {loading ? (
                                    ...
                                    <div className="p-6">
                                        <div className="flex mb-4">
                                            {/* <img src={`https://example.com/animals/${animal?.id}/image`} alt={animal?.nom} className="w-64 h-64 object-cover mr-4" /> */}
                                            <img src={`https://media.4-paws.org/1/c/e/f/1cefbff8cb079a4864a0745d0daf040baf2cd50a/IMG_20190721_172034-2976x2060-1920x1329.jpg`} alt={animal?.nom} className="w-64 h-64 object-cover mr-4" />
                                            <div>
                                                <p>
                                                    <strong>Nom :</strong> {animal?.nom}
                                                </p>
                                                <p>
                                                    <strong>Espece :</strong> {animal?.espece}
                                                </p>
                                                <p>
                                                ...
                                                <p>
                                                    <strong>Poids :</strong> {animal?.poids} kg
                                                </p>
                                                <p>
                                                    <strong>Taille :</strong> {animal?.taille} cm
                                                </p>
                                                <p>
                                                    <strong>Couleur :</strong> {animal?.couleur}
                                                </p>
                                                <p>
                ...
                <div className="lg:flex gap-4 items-stretch m-6">
                    ...
                    <div className="bg-white p-4 rounded-lg xs:mb-4 max-w-full shadow-md lg:w-[65%]">
                        <div className="flex flex-wrap justify-between h-full">
                            <div
                                className="flex-1 bg-gradient-to-r from-green-400 to-green-600 rounded-lg flex flex-col items-center justify-center p-4 space-y-2 border border-gray-200 m-2">
                            ...
                            <div className="relative flex flex-col min-w-0 break-words border border-dashed bg-clip-border rounded-2xl border-stone-200 bg-light/30">
                                <div className="px-9 pt-5 flex justify-between items-stretch flex-wrap min-h-[70px] pb-0 bg-transparent">
                                    <h3 className="flex flex-col items-start justify-center m-2 ml-0 font-medium text-xl/tight text-dark">
                                        <span className="mr-3 font-semibold text-dark">Liste des animaux</span>
                                        <Link to={{ pathname: "/pet/form", }} className="text-white p-button font-bold bg-cyan-600 p-2 mt-2">
                                            <FaPlus color="white" className="me-2" />  Ajouter un animal
                                        </Link>
                                    </h3>
                                ...
                                <DataTable className="flex-auto block py-8 pt-6 px-9 text-center" value={products} scrollable scrollHeight="400px" style={{ minWidth: '50rem' }}>
                                    <Column header="Nom" body={imageBodyTemplate}></Column>
                                    <Column field="type" header="Type"></Column>
                                    <Column field="race" header="Race"></Column>
                                    <Column field="age" header="Age"></Column>
                                    <Column field="height" header="Taille"></Column>
