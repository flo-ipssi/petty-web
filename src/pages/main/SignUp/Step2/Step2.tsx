...
const Step2: FC<Step2Props> = ({ data, onPrevious, onNext }) => {
  ...
  return (
    <div className="flex flex-col-reverse md:flex-row w-screen overflow-auto step2 ">
      <div
        className={`${data?.company ? "w-screen mx-auto" : "w-full md:w-5/6 mx-auto"
          }  bg-right grid grid-cols-1 md:grid-cols-2 gap-4 content-center relative`}
      >
        {data?.company == false ? (
        ...
        <div
          className={`${data?.company ? "w-screen mx-auto" : "w-4/6 mx-auto"}
    bg-right gap-4 content-center relative mb-52`}
        >
          <div className=" grid grid-cols-1">
            <h2 className="text-center w-full">Importez une photo de vous</h2>
            <div
              className="custum-file-upload-profil m-2 mt-8 justify-self-center"
              style={
                profileImage
                  ? {
                    backgroundImage: `url("${URL.createObjectURL(
              ...
              >
                ...
                {!profileImage && (
                  <div className="m-auto ">
                    <AiOutlineCloudUpload size={80} color={colors.OVERLAY} />
                  </div>
                )}
              </label>
            </div>
          ...
          <div className="mx-auto text-center">
            <button
              onClick={onPrevious}
              className="flex-shrink-0 mx-auto bg-red-500 hover:bg-red-700 border-red-500 hover:border-red-700 text-sm border-4 text-white py-1 px-2 rounded me-6"
              type="button"
            >
              Précèdent
            </button>
